...
import { User, Menu, X, MapPin, Info, Mail, LogOut, Bell, UserCircle } from 'lucide-react';
...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="w-full fixed top-0 z-50">
      {/* Top bar */}
      <div
        className={`hidden lg:block w-full overflow-hidden bg-brand-text dark:bg-brand-text transition-[max-height,opacity] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
          scrolled
      ...
      >
        <div
          className={`border-b border-brand-text/90 dark:border-brand-primary transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                ...
                {user ? (
                  <div className="relative" ref={userMenuRef}>
                    <button
                      ...
                      className="w-10 h-10 rounded-full overflow-hidden border-2 border-brand-primary/25 dark:border-brand-primary hover:border-brand-primary dark:hover:border-brand-primary/80 transition-colors"
                    >
                      {user.picture ? (
                        <img src={user.picture} alt={user.name} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                      ) : (
                        <div className="w-full h-full bg-brand-text dark:bg-brand-primary flex items-center justify-center">
                    ...
                    {showUserMenu && (
                      <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-brand-text rounded-lg shadow-2xl border border-brand-primary/20 dark:border-brand-primary py-2 z-50">
                  ...
                  <li key={item.name}>
                    <Link
                      ...
                      className={`block rounded-lg px-4 py-3 text-base font-medium transition-colors duration-200 ${
                        isActive(item.href)
                          ? 'bg-brand-primary/10 text-brand-primary dark:bg-brand-primary/25 dark:text-brand-primary/70'
                          : 'text-brand-text/90 hover:bg-brand-background hover:text-brand-primary dark:text-slate-200 dark:hover:bg-brand-text/90 dark:hover:text-brand-primary/70'
                      }`}
                      onClick={() => setIsOpen(false)}
                      aria-current={isActive(item.href) ? 'page' : undefined}
                    >
      ...
      {showUserMenu && user && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute top-20 right-4 left-4 sm:left-auto sm:w-80 bg-white dark:bg-brand-text rounded-lg shadow-2xl border border-brand-primary/20 dark:border-brand-primary p-2">
            <Link to="/profile" onClick={() => setShowUserMenu(false)}
