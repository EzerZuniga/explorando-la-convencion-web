      ...
      <div className="wp-section">
        <div className="wp-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {page.items.map((tipCategory, index) => (
              <div
                ...
                className="wp-card wp-card-interactive p-6 border-t-4 border-brand-primary animate-reveal-up"
                style={{ animationDelay: `${index * 0.08}s` }}
              >
                <h3 className="font-heading text-2xl font-semibold text-brand-text  mb-4 flex items-center">
                  <span className="mr-3 text-2xl">
                    {TIP_ICONS[tipCategory.category]}
                  </span>
                ...
                <ul className="space-y-3">
                  {tipCategory.tips.map((tip, tipIndex) => (
                    <li key={tipIndex} className="flex items-start">
                      <span className="text-brand-primary  mr-2 mt-1 font-bold">
                        •
                      </span>
                      <span className="text-brand-text/75 ">{tip}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="mt-16 wp-card p-8 text-center border-2 border-brand-primary/40">
            ...
            <p className="text-brand-text/75  mb-6 max-w-2xl mx-auto">
